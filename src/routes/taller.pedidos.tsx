        ...
        <ul className="space-y-3">
          {pedidos.map((p) => {
            ...
            return (
              <li key={p.id}>
                <Link
                  to="/taller/pedidos/$id"
                  params={{ id: p.id }}
                  className="flex items-center justify-between gap-3 rounded-lg border border-gray-800 bg-[oklch(0.14_0.04_250)] p-4 hover:border-emerald-600/40 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {total != null && (
                      <span className="text-sm font-medium text-gray-300">{formatoPrecioCop(total)}</span>
                    )}
